...
def sync_tx_to_neon(engine: Engine, tx_hash: str) -> dict:
    ...
    for lg in receipt.get("logs", []) or []:
        ...
        qty, _cost, first_id, last_id = abi_decode(
...
<div class="kpi">
  ...
  <div class="v">#{round_id}</div>
  <div class="s">{state_lbl(state)}</div>
</div>
""",
            unsafe_allow_html=True,
        )
    ...
    with k2:
        st.markdown(
            f"""
<div class="kpi">
  <div class="t">Prize Pool</div>
  <div class="v">{pool:,.1f} {sym}</div>
  <div class="s">Live contract balance</div>
</div>
""",
            unsafe_allow_html=True,
        )
    ...
    with k3:
        st.markdown(
            f"""
<div class="kpi">
  <div class="t">Tickets Sold</div>
  <div class="v">{sold:,}</div>
  <div class="s">Current round</div>
</div>
""",
            unsafe_allow_html=True,
        )
    ...
    with k4:
        st.markdown(
            f"""
<div class="kpi">
  <div class="t">Ticket Price</div>
  <div class="v">{rsnap["price_str"]}</div>
  <div class="s">USDT on BSC</div>
</div>
""",
            unsafe_allow_html=True,
        )
    ...
    with left:
        ...
        if last_winners and last_winners.get("winners"):
            st.markdown(
                f"""
<div class="card">
  <div class="muted">Last Draw Round</div>
  <h3 style="margin:4px 0 12px 0;">Round #{last_winners["round_id"]}</h3>
  <div class="muted">
    Prize paid: <b>{last_winners["pot_after_fee"]:,.4f} {sym}</b><br>
    Admin fee: <b>{last_winners["admin_fee"]:,.4f} {sym}</b><br>
    Emergency draw: <b>{"Yes" if last_winners["emergency"] else "No"}</b>
  </div>
</div>
""",
                unsafe_allow_html=True,
            ...
            for idx, ev in enumerate(last_winners["winners"], start=1):
                pct_value = fallback_pct[idx - 1] if idx - 1 < len(fallback_pct) else 0

                st.markdown(
                    f"""
<div class="winner-card">
  <div class="winner-rank">Winner #{idx} · Ticket #{ev["ticket_id"]} · {pct_value}%</div>
  <div class="winner-amt">{ev["amount"]:,.4f} {sym}</div>
  <div class="winner-meta">Wallet: <b>{fmt_addr(ev["winner"])}</b></div>
</div>
""",
                    unsafe_allow_html=True,
                )
    ...
    with right:
        ...
        st.markdown(
            f"""
<div class="card">
  <h3 style="margin-top:0;">Round #{round_id} is {state_lbl(state)}</h3>
  <div class="muted">
    Sales close:<br>
    <b>{rsnap["close_str"]}</b><br><br>
    Draw time:<br>
    <b>{rsnap["draw_str"]}</b>
  </div>
</div>
""",
        ...
        qty_preview = st.slider(
            "Preview tickets",
            min_value=1,
            max_value=int(MAX_TICKETS_PER_WALLET),
            value=1,
            help="This only previews estimated odds. Buying happens on the wallet buy page.",
        )

        future_total = max(1, sold + qty_preview)
        preview_odds = pct(qty_preview, future_total)
        preview_cost = (rsnap["ticket_price_units"] / (10 ** int(snap["dec"]))) * qty_preview

        st.markdown(
            f"""
<div class="odds-hero">
  <div style="font-size:11px; letter-spacing:1px; font-weight:900; color:rgba(233,238,247,.70); text-transform:uppercase;">
    Estimated Chance
  </div>
  <div class="odds-num">{preview_odds:.2f}%</div>
  <div style="font-size:13px; color:rgba(233,238,247,.70); margin-top:6px;">
    {qty_preview} ticket(s) · estimated cost <b>{preview_cost:,.1f} {sym}</b>
  </div>
</div>
""",
            unsafe_allow_html=True,
        ...
        st.link_button("🦊 Buy Tickets with MetaMask", BUY_DAPP_URL, use_container_width=True)
    ...
    try:
        ...
        if rows:
            ...
            st.info("No current-round purchases synced yet. Purchases will appear here after Neon sync.")
    except Exception as e:
        st.warning(f"Could not load live activity from Neon: {e}")
...
st.write(
    f"Round: **#{round_id}** · Status: **{state_lbl(state)}** · Pool: **{pool:,.2f} {sym}** · Ticket Price: **{rsnap['price_str']}**"
)
...
if sold > 0 and leaderboard_rows:
    ...
    for r in leaderboard_rows[:5]:
        qty = int(r[1])
        st.markdown(
            odds_badge_html(fmt_addr(r[0]), qty, pct(qty, sold), max_qty),
            unsafe_allow_html=True,
        )
else:
